        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-blue-50/60 p-3 rounded-lg border border-blue-100 flex items-start gap-2.5">
            ...
            <div className="text-xs">
              <span className="font-semibold text-slate-800">
                Ghi nhận người thực hiện sửa đổi:
              </span>
              {currentUser ? (
                <div className="text-slate-600 mt-0.5">
                  <span className="font-medium text-blue-800">{currentUser.name}</span>
                  {' · '}
                  <span className="font-mono bg-blue-100/80 px-1 py-0.2 rounded text-[11px] font-semibold text-blue-700">
                    Mã NV: {currentUser.employeeId}
                  </span>
                  {' · '}
                  <span className="text-slate-500">{currentUser.unit}</span>
                </div>
              ) : (
          ...
          {row.lastEditedBy && (
            <div className="text-[11px] text-slate-500 flex items-center gap-1.5 pt-1">
              ...
              <span>Lần sửa gần nhất bởi: </span>
              <strong className="text-slate-700">{row.lastEditedBy.userName} ({row.lastEditedBy.userCode})</strong>
              <span>- {row.lastEditedBy.timestamp}</span>
            </div>
          )}
